...
<div class="container-fluid">
    <div class="row">
       ...
       <div class="card-header"><h3>{{$title}}   <a href="{{url(route('vendor_packagesadd_management', $slug))}}" class="add_btn"><i class="fa fa-plus"></i></a></h3></div>
           <div class="card-body">
  ...
  <div class="acrdn-card">
    <div class="acrdn-header" id="headingOne">
        <button class="d-flex align-items-center justify-content-between btn btn-link collapsed" data-toggle="collapse" data-target="#{{$k}}" aria-expanded="false" aria-controls="collapseOne">          
          ...
          </span><!-- <span class="question-count">{{($k + 1)}}</span> --> {{$f->title}} ({{ $f->amount }})</h3>          
        </button>
        <ul class="acrdn-action-btns">
          <li><a href="{{ route('vendor_packagesedit_management', ['slug' => $category->slug, 'id' => $f->id]) }}" class="action_btn primary-btn"><i class="fas fa-pencil-alt"></i></a></li>
